...
const Register = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(getErrorMessage(requestError, "Unable to register. Please try again."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        <label className="block">
          ...
          <div className="relative">
            <User className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-500 dark:text-slate-300" />
            <input
              type="text"
              required
              value={name}
          ...
          {submitted && fieldErrors.name ? (
            <p className="mt-1 text-xs text-rose-200">{fieldErrors.name}</p>
          ) : null}
        </label>
      ...
      {success ? (
        <p className="mt-4 rounded-lg border border-emerald-400/40 bg-emerald-500/15 px-3 py-2 text-sm text-emerald-200">
          {success}
        </p>
      ) : null}
